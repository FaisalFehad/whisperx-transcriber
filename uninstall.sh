...
SHELL_RC="$HOME/.zshrc"
OBSIDIAN_RECORDINGS="$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Sessions/Recordings"
OBSIDIAN_SCRIPTS="$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Sessions/Scripts"
LAUNCHD_PLIST="$HOME/Library/LaunchAgents/com.transcriber.watch.plist"

echo ""
echo "┌─────────────────────────────────────────────┐"
echo "│  Local Transcriber — Uninstaller        │"
echo "└─────────────────────────────────────────────┘"
echo ""
...
if grep -q "alias transcribe=" "$SHELL_RC" 2>/dev/null; then
    sed -i '' '/# Local Transcriber/d' "$SHELL_RC"
...
if [ -d "$OBSIDIAN_RECORDINGS" ] || [ -d "$OBSIDIAN_SCRIPTS" ]; then
    ...
    if [ "$remove_obsidian" = "y" ] || [ "$remove_obsidian" = "Y" ]; then
        [ -d "$OBSIDIAN_RECORDINGS" ] && rm -rf "$OBSIDIAN_RECORDINGS" && echo "  ✅ Removed Obsidian/Sessions/Recordings"
        [ -d "$OBSIDIAN_SCRIPTS" ] && rm -rf "$OBSIDIAN_SCRIPTS" && echo "  ✅ Removed Obsidian/Sessions/Scripts"
    else
